...
export function TaskMasterProvider({ children }: { children: React.ReactNode }) {
  ...
  const refreshProjects = useCallback(async () => {
    if (IS_CODEX_ONLY_HARDENED || !user) {
      setProjects([]);
  ...
  const refreshTasks = useCallback(async () => {
    ...
    if (IS_CODEX_ONLY_HARDENED || !projectName || !user) {
      setTasks([]);
      setNextTask(null);
      return;
    }
  ...
  useEffect(() => {
    if (!isAuthLoading && user && !IS_CODEX_ONLY_HARDENED) {
  ...
  useEffect(() => {
    if (currentProject?.name && user && !IS_CODEX_ONLY_HARDENED) {
      void refreshTasks();
    }
